        ...
        if (timer > maxTime){
            ...
            if(randomNumber == 1){
                ...
                if(randomNumber == 1){
                    ...
                    new_sizeup.transform.position = transform.position + new Vector3(2, Random.Range(-3, 3), 0);
                    new_sizeup.transform.parent = new_obstacle.transform;
                    Destroy(new_sizeup, 12);
                }
                else{
                    GameObject new_health = Instantiate(health);
                    new_health.transform.position = transform.position + new Vector3(2, Random.Range(-3, 3), 0);
